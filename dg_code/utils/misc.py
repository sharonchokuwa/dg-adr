...
ALL_METHODS = ['GDRNet', 'ERM', 'MixupNet', 'Fishr', 'DRGen', 'DG_ADR', 'VAE_DG', 'SelfReg', 'SD',]
...
def update_writer(writer, epoch, loss_avg, loss_focal_avg=None, loss_alignment_avg=None):
    logging.info('epoch: {}, total loss: {}'.format(epoch, loss_avg.mean()))
    writer.add_scalar('info/total_loss', loss_avg.mean(), epoch)

    if loss_alignment_avg:
        logging.info('epoch: {}, loss_alignment: {}'.format(epoch, loss_alignment_avg.mean()))
        writer.add_scalar('info/loss_alignment', loss_alignment_avg.mean(), epoch)
    if loss_focal_avg:
        logging.info('epoch: {}, loss_focal: {}'.format(epoch, loss_focal_avg.mean()))
        writer.add_scalar('info/loss_focal_avg', loss_focal_avg.mean(), epoch)
